...
export default class ModMailClient extends Client {
    ...
    public async replyToDM(message: Message) {
        ...
        if (userGuilds.length == 1) {
            ...
            await message.reply(`Thank you for your inquiry, your ticket ID is WL-${mail.manager.total()}. The average response time is ${Duration.fromMillis(await this.mail.getAverageResponseTime(userGuilds[0].guild) * 1000).rescale().toHuman()}.`)
            return
        }
